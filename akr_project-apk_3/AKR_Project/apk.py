...
from apk_func import signEval, apkSignFile, addCertificate
# from entity import Entity
from functions import does_contain, entities_authorities_name_list
...
def signDocument():
    ...
    messagebox.showinfo("Signature", "File successfully signed")


signInputFile = Label(sign_tab, text="Path to file", width=40, height=2, borderwidth=1, relief="sunken")
signInputButton = Button(sign_tab, text="Choose file", command=choseInputSign, width=15, height=2)

entLabelSign = Label(sign_tab, text="Choose Entity:", width=40, height=2, borderwidth=1, relief="sunken")
...
def create_certificate():
    ...
    messagebox.showinfo("Create Certificate", "Successful certificate creation")


authLabel = Label(certificate_tab, text="Choose Authority:")
chosenAut = StringVar()
chosenAut.set(authList[0])
dropMenuAut = OptionMenu(certificate_tab, chosenAut, *authList)

entLabel = Label(certificate_tab, text="Choose Entity:")
chosenEnt = StringVar()
...
def choseSignCheck():
    signEval()
    if (signEval() != None):
        checkAnswer = Label(check_tab,text= "The document is: \nSigned by Entity: "+str(signEval().sign["Entity"])+"\nSignature: "
                                                +str(signEval().sign["EntitySign"])+ "\nCertified by Authority: "
                                                +str(signEval().sign["Authority"])+"\nCertificate: "+str(signEval().sign["Certificate"]),relief="sunken")
    elif(signEval()== None):
        checkAnswer = Label(check_tab, text= "Document is not signed!",relief="sunken")
    checkAnswer.grid(row=2, column=0)


checkInputButton = Button(check_tab, text="Choose file", command=choseInputCheck, width=20, height=2)
    ...
    if (not userInput.get()):
        ...
        if radioValue.get()==1:
            ...
            if userInput.get():
                ...
                if(does_contain(userInput.get(), file)):
                    messagebox.showinfo("Error!", "Authority/Entity already exists")
                    return
                #authority = Authority(str(userInput.get())) 
                messagebox.showinfo("Success", "Successfull creation of authority: " + userInput.get())
            ...
            if (userInput.get()):
                ...
                messagebox.showinfo("Success", "Successfull creation of entity: " + userInput.get())
        file.close()
        refreshValues()


textEnt = Label(add_tab, text="Fill the name of entity/ authority:")
addButton = Button(add_tab, text="Create", command=creation)
