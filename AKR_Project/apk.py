...
import main
from AKR_Project import functions
from main import apkSignFile, addCertificate
from authority import Authority
from entity import Entity
from functions import does_containt, entities_authorities_name_list, GenerateKeyPair, VerifySig_apk
...
def choseSignCheck():
    global checkAnswer
    checkAnswer.destroy()
    functions.field_metadata=VerifySig_apk(checkFile.cget("text"))
    if(functions.field_metadata == None):
        checkAnswer = Label(check_tab,text= "Error reading", relief= "sunken")
    elif (functions.field_metadata != False):
            checkAnswer = Label(check_tab,text= "Document: \nSigned by entity: "+functions.field_metadata[0] + "\nCertificated by authority: "
            +functions.field_metadata[1]+ "\nValid from: " +functions.field_metadata[2]+ "\nValid to: " +functions.field_metadata[3],relief="sunken")
    elif(functions.field_metadata == False):
        checkAnswer = Label(check_tab, text= "Document is not signed!",relief="sunken")
    checkAnswer.grid(row=2, column=0)

checkAnswer = Label(check_tab)
...
def creation():
    ...
    if (not userInput.get()):
        messagebox.showerror("Error", "Fill in the title")
...
textEnt = Label(add_tab, text="Enter title of entity/authority:")
